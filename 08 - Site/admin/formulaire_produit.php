...
if (!empty($_POST)) { // si le formulaire a été envoyé
    // ici il faudrait mettre tous les contrôles sur le formulaires

    $photo_bdd = ''; // par défaut la photo en BDD est vide

    // 9 suite : modification de la photo :
    if(isset($_POST['photo_actuelle'])) {   // si existe "photo_actuelle" dans $_POST c'eest que nous sommes en train de modifier le produit avec sa photo. On remet l'URL de la photo en BDD
        $photo_bdd = $_POST['photo_actuelle'];
    }

    // 5 suite : traitement de la photo :
    // debug($_FILES); // $_FILES est une superglobale qui représente l'input type "file" d'un form. L'indice "photo" correspond à l'attribut "name" de l'input. Les autres indices du sous-tableau sont prédéfinies : "name" pour le nom du fichier, "type" pour le type du fichier, "tmp_name" pour l'adresse du fichier temporaire en cours d'upload, "error" pour le code erreur de téléchargement, "size" pour la taille du fichier uploadé
    
    if(!empty($_FILES['photo']['name'])) {  // si un fichier est en cours d'upload
        $fichier_photo = $_FILES['photo']['name'];  // nom de la photo
        $photo_bdd = 'photo/' . $fichier_photo; // chemin relatif de la photo qui enregistré en BDD et qui nous servira pour l'attibut "src" des balises images (les photos sont copiées dans le dossier "photo" ligne suivante)
    ...
    if($requete) {  // si executeRequete a retourné un objet PDOStatement donc implicitement évalué à true, alors c'est que la requete a marchée
...
// 8- Remplissage formulaire de modification
// debug($_GET);
if(isset($_GET['id_produit'])){ // si "id_produit" est dans l'URL, c'est que nous avons demandé la modification du produit : on séléctionne les infos du produit en BDD pour l'afficher dans le formulaire
...
<form action="" method="post" enctype="multipart/form-data">
    <!-- l'attribut enctype spécifie que le formulaire envoie des données binaires(fichier) et du texte (champs de formulaire): permet d'uplodaer une photo -->

    <input type="hidden" name="id_produit" value="<?php  echo  $produit['id_produit'] ?? 0; ?>"> <!-- ce champ est nécessaire pour la modification ou la création du produit, car on a besoin de l'id pour la requete SQL. On met 0 par défaut en value pour que le REPLACE se comporte comme un INSERT et insère le produit en BDD. type "hidden" pour cacher le champ.    Ou on met l'ID du produit existant pour que le replace se comporte comme un UPDATE-->

    <div><label for="reference">Référence</label></div>
    <div><input type="text" id="reference" name="reference" value="<?php  echo  $produit['reference'] ?? ''; ?>"></div>

    <div><label for="categorie">Catégorie</label></div>
    <div><input type="text" id="categorie" name="categorie" value="<?php  echo  $produit['categorie'] ?? 0; ?>"></div>

    <div><label for="titre">Titre</label></div>
    <div><input type="text" id="titre" name="titre" value="<?php  echo  $produit['titre'] ?? ''; ?>"></div>
    ...
    <div><input type="text" id="couleur" name="couleur" value="<?php  echo  $produit['couleur'] ?? ''; ?>"></div>
    ...
    <div><input type="text" id="prix" name="prix" value="<?php  echo  $produit['prix'] ?? ''; ?>"></div>

    <div><label for="stock">Stock</label></div>
    <div><input type="text" id="stock" name="stock" value="<?php  echo  $produit['stock'] ?? ''; ?>"></div>
