...
if(isset($_GET['action']) && $_GET['action'] == 'deconnexion'){ // si le membre à cliqué sur "déconnexion"
    //debug($_SESSION);
    unset($_SESSION['membre']); // on ne fait pas un session_destroy() qui supprimerait toute la session car on veut conserver le panier de l'internaute
    $message = '<div class="alert alert-info">Vous êtes déconnecté</div>';
}

// 3 - Le membre déjà connecté est redirigé vers son profil :
if(estConnecte()) {
    header('location:profil.php'); //  nous affichons le form de connexion qu'aux membre connectés les autres sont redirigés vers le profil
...
if (!empty($_POST)) { // si le form a été envoyé
    ...
    if (empty($contenu)) {   // si c'est vide c'est qu'il y a pas d'erreur
        $resultat = executeRequete("SELECT * FROM membre WHERE pseudo = :pseudo", array(':pseudo' => $_POST['pseudo']));

        if ($resultat->rowCount() == 1) { // s'il ya 1 ligne de résultat, alors le pseudo existe on peut vérifier le mdp

            $membre = $resultat->fetch(PDO::FETCH_ASSOC); // on "fetche" l'objet $resultat pour en extraire les données du membre (pas de boucle car le pseudo ets unique)
            debug($membre);
            // on vérifie le mdp
            if (password_verify($_POST['mdp'], $membre['mdp'])) { // si le mdp du form correspond au hach du mdp en BDD alors cette fonction retourne true
                ...
                // puis on regirige vers la page de profil
